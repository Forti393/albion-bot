...
def get_main_kb(d):
    ...
    return ReplyKeyboardMarkup(keyboard=[
        ...
        [KeyboardButton(text="💰 Налаштувати бюджет"), KeyboardButton(text="🔄 Скинути")],
        ...
        for k, c_d in grouped.items():
            ...
            for sc in srcs:
                ...
                for tc in targets:
                    ...
                    if p_n >= p_l:
                        bd, sd = c_d[sc]['sell_price_min_date'], c_d[tc].get(sk)
                        b_dt = datetime.fromisoformat(bd.split(".")[0].replace("Z","")).replace(tzinfo=UTC)
                        s_dt = datetime.fromisoformat(sd.split(".")[0].replace("Z","")).replace(tzinfo=UTC)
                        ...
                        item_entry = {'id':i_id,'q':int(q),'from':sc,'to':tc,'buy':buy,'sell':sell,'p_p':int(sell*0.935-buy),'p_n':p_n,'bd':bd,'sd':sd}
                        
                        if check_liq:
                            vol = await get_item_liquidity(i_id.split("@")[0], tc)
                            if vol >= 1: # Фільтр: мінімум 1 продаж за сьогодні
                                item_entry['vol'] = vol
                                res.append(item_entry)
                        else:
                            res.append(item_entry)
...
async def disp_res(msg, res, d):
    ...
    for i in range(0, len(res[:15]), 5):
        ...
        for idx, r in enumerate(chunk, i+1):
            ...
            text += (f"{idx}) {get_item_icon(b_id)} <b>{name}</b> [{b_id.split('_')[0][1:]}.{r['id'].split('@')[1] if '@' in r['id'] else '0'}]\n"
...
# --- Далі йдуть твої стандартні хендлери без змін ---
...
async def main_search(m, state: FSMContext):
    ...
    try:
        ...
        await s_msg.delete(); await disp_res(m, res, d)
...
async def numeric_handler(m, state: FSMContext):
    ...
    try:
        ...
        if "buy_limit" in str(curr): await state.update_data(buy_limit=v); await state.set_state(None); await m.answer(f"✅ Бюджет: {v:,}", reply_markup=get_main_kb(await state.get_data()))
            ...
            d = await state.get_data(); await state.set_state(None); p_n = int((v*0.895)-d['b'])*d['c']
...
async def limit_menu(m, state: FSMContext):
    await m.answer("⚙️ Бюджет:", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="💰 Встановити", callback_data="set_limit_buy")]]))
...
async def cmd_start(m, state: FSMContext):
    await state.clear(); await m.answer("🚀 Готовий!", reply_markup=get_main_kb({}))
