            ...
            for key_id, entries in grouped_data.items():
                ...
                for entry_from in entries:
                    for entry_to in entries:
                        ...
                        if is_fake(item_type, 0, buy, sell, bd, sd): continue
...
async def start_cmd(message: types.Message, state: FSMContext):
    await message.answer("👋 Встанови ліміт на скупку (наприклад: 500000):", reply_markup=ReplyKeyboardRemove())
...
async def set_limit_start(message: types.Message, state: FSMContext):
    await message.answer("Введи нову максимальну суму за 1 предмет:", reply_markup=ReplyKeyboardRemove())
    await state.set_state(LimitState.waiting_for_limit)
...
async def set_limit_finish(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(f"✅ Ліміт: {MAX_BUY_PRICE:,} ср. Тисни Пошук!", reply_markup=keyboard)
...
async def scan_cmd(message: types.Message, state: FSMContext):
    ...
    if scan_running:
        ...
        return
    scan_running = True
    await message.answer(f"🔍 Шукаю фліпи (в т.ч. Black Market)...")
    results = await scan_market()
    if not results:
        await message.answer("Фліпів не знайдено.")
    else:
...
async def restart_cmd(message: types.Message):
    await message.answer("♻️ Оновлюю базу...")
