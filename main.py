        ...
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=30)) as session:
            async with session.get(ITEMS_URL) as resp:
                if resp.status == 200:
                    ...
                    items_data = {i["UniqueName"]: i for i in raw_data if i.get("UniqueName", "").startswith(("T4_", "T5_", "T6_", "T7_", "T8_")) and any(x in i.get("UniqueName", "").lower() for x in allowed)}
...
async def cmd_help(message: types.Message, state: FSMContext):
    await state.clear()
    await message.answer("📖 <b>Допомога:</b>\n\n1️⃣ ⚙️ Ліміт — бюджет та прибуток.\n2️⃣ 🔍 Пошук — старт сканування.\n3️⃣ 🗺️ Режими — вибір міст.\n4️⃣ ⚡ Екстра — швидкий ринок (<30хв).\n5️⃣ 🧮 Калькулятор — ручний підрахунок.", reply_markup=get_start_kb(), parse_mode=ParseMode.HTML)

@dp.message(F.text == "⚙️ Ліміт", StateFilter('*'))
async def limit_menu(message: types.Message, state: FSMContext):
    await state.clear()
    await message.answer("⚙️ Обери ліміт для налаштування:", reply_markup=get_limits_inline(), parse_mode=ParseMode.HTML)
...
async def main_search(message: types.Message, state: FSMContext):
    ...
    if max_buy_limit <= 0: return await message.answer("Спочатку встанови ліміт купівлі!")
    if current_mode is None: return await message.answer("Спочатку обери режим!", reply_markup=get_mode_inline())
    if current_mode == "all":
...
async def menu_back(message: types.Message, state: FSMContext):
    await state.clear(); await message.answer("Головне меню:", reply_markup=get_main_kb())

@dp.message(F.text.startswith("🗺️ Режими"), StateFilter('*'))
async def modes_btn(message: types.Message, state: FSMContext):
    await state.clear(); await message.answer("Режими:", reply_markup=get_mode_inline())

@dp.message(F.text.contains("Екстра"), StateFilter('*'))
async def toggle_extra(message: types.Message, state: FSMContext):
    await state.clear()
    global extra_filter_active; extra_filter_active = not extra_filter_active
...
async def calc_init(message: types.Message, state: FSMContext):
    await state.clear()
    await message.answer("📦 Кількість предметів:", reply_markup=ReplyKeyboardRemove())
...
# ================= CALLBACKS =================
@dp.callback_query(F.data == "admin_update", StateFilter('*'))
async def do_admin_update(callback: types.CallbackQuery):
    if callback.from_user.id == ADMIN_ID:
        await callback.message.edit_text("⏳ Завантажую нові дані...")
        asyncio.create_task(download_items())
        await callback.message.edit_text("✅ Базу оновлено!")
    await callback.answer()

@dp.callback_query(F.data == "confirm_restart", StateFilter('*'))
async def do_restart_yes(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("🔄 Прогрес скинуто! Почни з <b>⚙️ Ліміт</b>.", reply_markup=get_start_kb(), parse_mode=ParseMode.HTML)
    await callback.answer()

@dp.callback_query(F.data == "cancel_restart", StateFilter('*'))
async def do_restart_no(callback: types.CallbackQuery, state: FSMContext):
    await state.clear(); await callback.message.delete()
    await callback.message.answer("❌ Скасовано.", reply_markup=get_main_kb())
    await callback.answer()
...
async def set_limit_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    if l_type == "buy":
        await callback.message.edit_text("💰 Вкажи ціну покупки за 1 шт:")
        await state.set_state(BotState.waiting_for_buy_limit)
    else:
        await callback.message.edit_text("📈 Вкажи мінімальний прибуток:")
...
async def set_mode(callback: types.CallbackQuery, state: FSMContext):
    global current_mode; current_mode = callback.data.split("_")[2]; await state.clear()
    if current_mode == "all": await callback.message.answer("✅ Режим 'Всі міста'! Тисни 🔍 Пошук", reply_markup=get_main_kb())
...
async def to_city(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"🚀 {d['f_c']} ➔ {callback.data.split('_')[1]}")
    res = await scan_logic(d['f_c'], callback.data.split("_")[1])
...
async def handle_buy_limit_input(message: types.Message, state: FSMContext):
    ...
    if val is not None:
        ...
        if current_mode is None: await message.answer("✅ Ліміт збережено! Тепер обери режим:", reply_markup=get_mode_inline())
        else: await message.answer(f"✅ Ліміт {max_buy_limit:,} збережено!", reply_markup=get_main_kb())
    else: await message.answer("❌ Введи коректне число!")
...
async def handle_profit_limit_input(message: types.Message, state: FSMContext):
    ...
    if val is not None:
        ...
        if current_mode is None: await message.answer("✅ Прибуток збережено! Тепер обери режим:", reply_markup=get_mode_inline())
        else: await message.answer(f"✅ Мін. прибуток {min_profit_limit:,} збережено!", reply_markup=get_main_kb())
    else: await message.answer("❌ Введи коректне число (можна з мінусом)!")

@dp.message(StateFilter(BotState.calc_count))
async def calc_cnt(message: types.Message, state: FSMContext):
    val = to_int(message.text)
    if val is not None and val > 0:
        await state.update_data(cnt=val); await message.answer("💰 Введи ціну КУПІВЛІ (1 шт):"); await state.set_state(BotState.calc_buy)
    else: await message.answer("❌ Введи додатнє число!")

@dp.message(StateFilter(BotState.calc_buy))
async def calc_b(message: types.Message, state: FSMContext):
    val = to_int(message.text)
    if val is not None:
        await state.update_data(b=val); await message.answer("📤 Введи ціну ПРОДАЖУ (1 шт):"); await state.set_state(BotState.calc_sell)
...
async def calc_s(message: types.Message, state: FSMContext):
    ...
    if val is not None:
        ...
        await message.answer(f"📊 <b>Результат для {cnt} шт:</b>\n👑 П: <b>{(int(s*0.935-b)*cnt):,}</b>\n💀 Б: <b>{(int(s*0.895-b)*cnt):,}</b>", reply_markup=get_main_kb(), parse_mode=ParseMode.HTML)
        await state.clear()
    else: await message.answer("❌ Введи число!")
