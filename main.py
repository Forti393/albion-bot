...
async def get_item_liquidity(item_id, city, quality):
    ...
    url = f"https://europe.albion-online-data.com/api/v2/stats/history/{item_id}?locations={city}&qualities={quality}&time-scale=24"
                ...
                if resp.status == 200:
                    ...
                    if data and isinstance(data, list) and len(data) > 0:
                        ...
                        for day in history:
                            ...
                            price_sum += p * v
                        if vol_24h > 0:
                            avg_p = int(price_sum / vol_24h)
                        else:
                            avg_p = 0
                        history_cache[cache_key] = {'volume': vol_24h, 'avg_p': avg_p, 'time': now}
                        return vol_24h, avg_p
        except Exception: pass
...
async def scan_logic(d, f_c=None, t_c=None):
    ...
    pre_res = []; b_l = d.get("buy_limit", 0); p_l = d.get("profit_limit", 1000)   # дефолт 1000
    ...
    for item in pre_res[:150]:
        ...
        # Викидаємо явні «пастки» — ціна > 3x історії
        if avg_p > 0 and item['sell'] > (avg_p * 3):
            continue

        # Фільтр попиту тільки якщо увімкнено
        if check_liq_limit and vol < 10:
            continue
...
async def limit_menu(m, state: FSMContext):
    d = await state.get_data()
    await m.answer(f"⚙️ Бюджет: {d.get('buy_limit',0):,}\n📈 Профіт: {d.get('profit_limit',1000):,}", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
